...
methods <- c("HGCGMA", "NPN-Glasso", "Glasso", "MAEGG")
...
for (p_val in ps) {
  ...
  if (p_val == 30) {
    y_upper <- auto_ylim[["30"]][2]
    y_lower <- min(d$mean - d$sd, na.rm = TRUE)  
  } else if (p_val == 50) {
    
    y_upper <- auto_ylim[["50"]][2]
    y_lower <- auto_ylim[["50"]][1]
  } else if (p_val == 100) {
    
    y_upper <- auto_ylim[["100"]][2]-1
...
print(combined_plot)
#ggsave("KL_Gaussian.jpeg", combined_plot, width = 12, height = 4, dpi = 300)
